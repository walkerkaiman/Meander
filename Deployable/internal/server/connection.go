...
type PlaybackErrorMessage struct {
	Type      string             `json:"type"`
	DeviceID  string             `json:"device_id"`
	Action    types.EngineAction `json:"action"`
...
func (c *Client) Run(ctx context.Context, helloProvider func() HelloMessage, incoming chan<- Incoming, outgoing <-chan any, connected chan<- time.Time) {
	backoff := time.Second
	for {
		select {
		case <-ctx.Done():
